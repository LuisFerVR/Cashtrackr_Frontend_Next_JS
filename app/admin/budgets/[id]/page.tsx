...
export default async function BudgetDetailsPage({
  ...
  const budget = await getBudgetById(params.id);
  const totalSpent = budget.expenses.reduce((total, expense) => {
      ...
      {budget.expenses.length ? (
        <>
          <div className="grid grid-cols-1 md:grid-cols-2 mt-10">
            ...
            <div className="flex flex-col gap-y-5 justify-center items-center md:items-start gap-5">
              <Amount 
                label="Presupuesto"
          ...
          <h1 className="font-blac text-4xl text-purple-950 mt-10">
        ...
        <>
          <p className="text-center py-20">No hay gastos en este presupuesto</p>
        </>
      )}
